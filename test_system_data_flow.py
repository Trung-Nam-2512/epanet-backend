#!/usr/bin/env python3
"""
Test SCADA API và phân tích dữ liệu đầu vào/đầu ra
"""
...
def test_scada_api():
    """Test SCADA API"""
    print("=" * 80)
    print("TEST SCADA API VA PHAN TICH DU LIEU")
    print("=" * 80)
    ...
    try:
        # Test SCADA data endpoint
        scada_params = {
            "station_codes": ["13085"],
            "hours_back": 24
        }
        
        scada_response = requests.post(
            "http://localhost:8000/api/v1/scada/data",
            json=scada_params,
            headers={"Content-Type": "application/json"}
        )
        
        print(f"SCADA API Status: {scada_response.status_code}")
        
        if scada_response.status_code == 200:
            scada_data = scada_response.json()
            print(f"SCADA API Response: SUCCESS")
            ...
            print(f"Response: {scada_response.text}")
    ...
    try:
        ...
        print(f"Simulation API Status: {sim_response.status_code}")
        
        if sim_response.status_code == 200:
            sim_data = sim_response.json()
            print(f"Simulation API Response: SUCCESS")
    ...
    print("\n3. PHAN TICH LOGIC HE THONG:")
    print("-" * 50)
    
    print("DU LIEU DAU VAO:")
    print("  - SCADA Pressure: Ap luc thuc te tu tram (boundary condition)")
    print("  - SCADA Flow: Luu luong thuc te tu tram (reference)")
